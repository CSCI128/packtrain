...
public interface CredentialRepo extends CrudRepository<Credential, UUID> {
    ...
    @Query("select c from credential c where c.owningUser.cwid = ?1 and c.type = ?2 and c.isPrivate = true and c.isActive")
    List<Credential> getByCwidAndEndpoint(String owningUserCwid, CredentialType type);

    @Query("select c from credential c join course_credential cc on cc.credential.id=c.id where c.isPrivate = false and c.isActive= true and cc.isActive = true and cc.course.id = ?1 and c.type = ?2")
    ...
    @Query("select count(c) > 0 from credential c where c.owningUser.cwid = ?1 and c.type = ?2 and c.isActive = true")
