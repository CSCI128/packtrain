import {Button, Center, Container, Divider, Group, Table, Text} from "@mantine/core";
...
export function CoursePage() {
  ...
  const { data: courseData, error: courseError, isLoading: courseIsLoading } = $api.useQuery(
    "get",
  ...
  const {data: policyData, error: policyError, isLoading: policyIsLoading} = $api.useQuery(
    "get",
  ...
  const [policyDataState, setPolicyData] = useState<components['schemas']['Policy'][]>(policyData || []);
  ...
  const handlePolicyDelete = (element: components['schemas']['Policy']) => {
    ...
    userManager.getUser().then(u => {
      axios.delete(`/api/instructor/course/${store$.id.get()}/policies/${element.id}`,
        {
          headers: {
  ...
  const rows = policyDataState.map((element) => (
    <Table.Tr key={element.name}>
      ...
      <Table.Td><a href={element.uri}>S3 URI</a></Table.Td>
    ...
    <>
      <Container size="md">
        ...
        <Text size="md" onClick={() => navigate("/admin/assignments")}>
          <b>{courseData.assignments?.length}</b> assignments <BsBoxArrowUpRight />
        </Text>
