"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.downloadAndVerifyPolicy = downloadAndVerifyPolicy;
const axios_1 = __importDefault(require("axios"));
const RawScoreDTO_1 = __importDefault(require("../data/RawScoreDTO"));
function verifyPolicy(fun) {
    const rawScore = new RawScoreDTO_1.default();
    rawScore.cwid = "10000";
    rawScore.extensionDate = new Date();
    rawScore.submissionDate = new Date();
    rawScore.extensionId = "1";
    rawScore.assignmentId = "1";
    rawScore.rawScore = 10;
    rawScore.extensionHours = 0;
    rawScore.extensionType = "Late Pass";
    try {
        const f = fun(rawScore);
        return rawScore.cwid === f.cwid && rawScore.extensionId === f.extensionId && rawScore.assignmentId === f.assignmentId;
    }
    catch (e) {
        return false;
    }
}
function downloadAndVerifyPolicy(uri) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield axios_1.default.get(uri);
        const f = Function(res.data);
        if (!verifyPolicy(f)) {
            return Promise.reject("Invalid policy!");
        }
        return f;
    });
}
