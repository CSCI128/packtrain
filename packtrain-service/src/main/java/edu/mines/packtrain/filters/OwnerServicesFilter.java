...
@Slf4j
public class OwnerServicesFilter implements HandlerInterceptor {
    private final SecurityManager securityManager;

    public OwnerServicesFilter(SecurityManager securityManager) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        log.debug("Validating owner access to course for user '{}'",
        ...
        if (!securityManager.hasCourseMembership(CourseRole.OWNER, UUID.fromString(courseId))) {
            log.warn("Blocked attempt by '{}' to access course '{}' as an owner!",
