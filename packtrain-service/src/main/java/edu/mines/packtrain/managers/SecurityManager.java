...
public class SecurityManager implements IdentityProvider {
     ...
     * At a high level, this attempts to look up the user by their oauth user id
     * (the UUID that Authenik assigns them).
     * <br></br>
     * If that fails (the account may have been created via a canvas import
     * or manually via the /admin/users route),
     * then attempt to look up the user by their cwid.
     * <br></br>
     * If that fails, then this is a brand-new user, and we need to create
     * their corresponding account in the db.
     * <br></br>
     ...
     */
    private Optional<User> attemptToCreateNewUser(final Jwt token, final String oauthId,
                                                  final String cwid) throws AccessDeniedException {
    ...
    public boolean hasCourseMembership(CourseRole role, UUID courseId) {
        ...
        log.debug("Verifying '{}' has access to course '{}' with role '{}'",
                user.getEmail(), courseId, role.getRole());
