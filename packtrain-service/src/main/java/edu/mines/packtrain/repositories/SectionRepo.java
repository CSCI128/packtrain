...
public interface SectionRepo extends CrudRepository<Section, UUID> {
    ...
    // this is so cursed
    @Query(value = "select s.id as id, s.name as name, s.canvas_id as canvas_id, s.course_id " +
            "as course_id from member_section as ms " +
            "join sections as s on s.id = ms.section_id " +
            "where ms.member_id = ?1", nativeQuery = true)
