...
# Admin UI route
resource "aws_lb_listener_rule" "frontend_admin_rule" {
...
# Student UI route
resource "aws_lb_listener_rule" "frontend_student_rule" {
  listener_arn = aws_lb_listener.app_listener.arn
  priority     = 20
...
# Instructor UI route
resource "aws_lb_listener_rule" "frontend_instructor_rule" {
  listener_arn = aws_lb_listener.app_listener.arn
  priority     = 30
...
# Authentik route
resource "aws_lb_listener_rule" "authentik_rule" {
  listener_arn = aws_lb_listener.app_listener.arn
  priority     = 40
...
resource "aws_lb_listener_rule" "policy_rule" {
  listener_arn = aws_lb_listener.app_listener.arn
  priority     = 50
...
resource "aws_s3_bucket_ownership_controls" "grading_controls" {
...
resource "aws_s3_bucket_versioning" "grading_versioning" {
  bucket = aws_s3_bucket.grading_bucket.id
...
resource "aws_s3_bucket_lifecycle_configuration" "grading_lifecycle" {
  bucket = aws_s3_bucket.grading_bucket.id
...
resource "aws_mq_broker" "rabbitmq_broker" {
  ...
  engine_version             = "3.13" # adjust version as desired
  auto_minor_version_upgrade = true
  deployment_mode            = "SINGLE_INSTANCE" # Multi-AZ deployment
  host_instance_type         = "mq.t3.micro"
  publicly_accessible        = false
  subnet_ids                 = [module.vpc.public_subnets[0]] # Use public subnets created by the VPC module
