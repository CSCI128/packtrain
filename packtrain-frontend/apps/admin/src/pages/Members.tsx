...
export function MembersPage() {
  const { data, error, isLoading } = useGetMembers(["students"], "getStudents");

  const {
    data: instructorData,
    error: instructorError,
    isLoading: instructorIsLoading,
  } = useGetMembers(["instructors"], "getInstructors");
  ...
  const instructorRows = sortedDataInstructor.map((row) => (
    <Table.Tr key={row.cwid}>
      <Table.Td>{row.name}</Table.Td>
      <Table.Td>{row.cwid}</Table.Td>
